...
#### ADFA
```{r}
out.lm<-lm(PC3 ~ lfm, data = seg.adfa.subset.p)
davies.test(out.lm, ~lfm, k = 50)
...
segplot(seg.adfa.subset.p, lfm, PC3, fit, mean_lfm_adfa, lwr_lfm_adfa, upr_lfm_adfa, lower, upper)
```

#### CEME
```{r}
out.lm<-lm(PC3 ~ lfm, data = seg.ceme.subset.p)
davies.test(out.lm, ~lfm, k = 50)
...
segplot(seg.ceme.subset.p, lfm, PC3, fit, mean_lfm_ceme, lwr_lfm_ceme, upr_lfm_ceme, lower, upper)
```
