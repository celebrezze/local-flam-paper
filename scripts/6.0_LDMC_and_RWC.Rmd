...
mem.table.df %>%
  ...
  kable(format = 'html', escape = F, col.names = c('Dependent Variable', 'Model Fixed Effects', 'Hydration', "Dry Matter", '\U0394 AIC', 'AIC', expression('Marg. R2/Cond. R2'))) %>% 
  kable_styling(bootstrap_options = c('hover', 'bordered', 'condensed'), fixed_thead = T) %>% 
  row_spec(c(1, 4, 6, 10), background = "#e5e5e5", bold = T, font_size = 16) %>% 
  row_spec(c(0), background = "lightgrey", bold = T, font_size = 18) %>% 
