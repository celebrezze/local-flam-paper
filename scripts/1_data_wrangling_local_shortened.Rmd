...
dlookr::diagnose(raw.data) #take a look at it

#kbl(raw.data) %>% kable_styling(bootstrap_options = c("striped")) 
```
...
data.with.na.ceme %>% 
ggplot() +
  geom_point(aes(y = lfm, x = mpa), color = "blue") +
  geom_point(aes(y = lfm.outliers.out, x = mpa), color = "goldenrod")
```
```{r}
...
ggplot(aes(y = lfm.NAs.imputed, x = mpa, color = year.month)) +
  geom_point(size = 1) +
  facet_wrap(~spp) 
```
...
```{r}
data.5bins %>%
  filter(ignition == 1, year != 2016) %>%
ggplot(aes(y = prop.ignite.bins5, x = lfm, color = year.month, shape = model)) +
  geom_point(size = .5) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~spp)
```
...
data.5bins_nodate %>%
  filter(ignition == 1, year != 2016) %>%
ggplot(aes(y = prop.ignite.bins5_nodate, x = lfm, shape = model)) +
  geom_point(size = .5) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~spp)
```
...
data.10bins_nodate %>%
  filter(ignition == 1, year != 2016) %>%
ggplot(aes(y = prop.ignite.bins10_nodate, x = lfm, shape = model)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~spp)
```
...
data %>% 
  filter(model == "EPI") %>% 
ggplot() +
  geom_point(aes(y = ww.flam.sample, x = mpa), color = "blue") +
  geom_point(aes(y = ww.outs, x = mpa), color = "goldenrod")
```
...
# 
# for(k in groupmodel) {
#   data.test.2 <- if(k == 'EPI') filter(data, ww.flam.sample < 0.75)
#           else if(k == 'HP') filter(data, ww.flam.sample < 1.6)
# }

# data.test <- data %>% 
#    filter(case_when(model == "EPI" ~ ww.flam.sample < .75, 
#                     model == "HP" ~ ww.flam.sample < .1.6)
#          ) 
...
# for(model in groupkind) {
#           if(model == 'EPI') outlierKD2(data, dw.outs)
#           else if(model == 'HP') outlierKD2(data, dw.outs)
# }
```
...
#read_csv(here("processed-data", "data.halfwayprocessed.csv"), show_col_types = FALSE) %>% 
  filter(spp == "CEME")
```
