...
precip_mod <- lmer(PC1 ~ mpa.unscaled + Species + season + site + (1 | individual), data = seg.data.subset.alldates)
...
```{r}
precip_mod.2 <- lmer(PC2 ~ mpa.unscaled + Species + season + site + (1 | individual), data = seg.data.subset.alldates)
...
Models:
```{r}
mpalfm_mod <- lmer(lfm ~ mpa + Species + season + site + (1 | individual), data = seg.data.subset.alldates)
...
###
mpadw_mod <- lmer(dw.flam.sample ~ mpa + Species + season + site + (1 | individual), data = seg.data.subset.alldates)
...
#### Flame duration
```{r}
mpa_mod_fig_fd <- lmer(fd ~ mpa.unscaled + Species + site + (1 | individual), data = scaled.mem.data.sept2020)
...
#### Glow duration
```{r}
mpa_mod_fig_gd <- lmer(gd ~ mpa.unscaled + Species + site + (1 | individual), data = scaled.mem.data.sept2020)
...
#### PC1
```{r}
mpa_mod_fig_pc1 <- lmer(PC1 ~ mpa.unscaled + Species + site + (1 | individual), data = scaled.mem.data.sept2020)
