...
fd.mpa <- ggplot() + 
  ...
  geom_smooth(method = "lm", data= fd_remef_year_df, aes(mpa.unscaled, fd_remef_year, color = Species), se = TRUE, size = 1, alpha = .5) +
...
gd.mpa <- ggplot() + 
  ...
  geom_smooth(method = "lm", data= gd_remef_year_df, aes(mpa.unscaled, gd_remef_year, color = Species), se = TRUE, size = 1, alpha = .5) +
   stat_regline_equation() +
  labs(
    x="Water Potential (-Mpa)", 
       y="Glow Duration (sec)")  +
...
pc1.mpa <- ggplot() + 
  ...
  geom_smooth(method = "lm", data= pc1_remef_year_df, aes(mpa.unscaled, pc1_remef_year, color = Species), se = TRUE, size = 1, alpha = .5) +
