...
seg.data.subset$color[seg.data.subset$year.month=='2020_January']='#669BBC'
seg.data.subset$color[seg.data.subset$year.month=='2019_December']="#b08ba5"
...
abline(v = tlp, col = alpha("#64A400", 0.7), cex = 3)
polygon(x = c(tlp.lower, tlp.upper, tlp.upper, tlp.lower), y = c(ymax +0.2*ymax, ymax +0.2*ymax, ymin -30, ymin -30), col = alpha("#64A400", 0.3), border = alpha("#64A400", 0.6))
legend('topleft', legend=if(species == 'ADFA'){
  ...
  c("September 2020", "January 2020", "December 2019")
       ...
       col=if(species == "ADFA") {
        ...
        c("#c1121f",'#669BBC',"#b08ba5")
        }, pch = 16, cex=1)
for(z in 1:length(xvar)){
  if(z < p){
    segments(x0 = 0, x1 = p, y0 = b, y1 = m*((b2 - b)/(m - m2))+b, lwd = 3, col = "black")
  }else{
    segments(x0 = p, x1 = max.x, y0 = m2*((b - b2)/(m2 - m))+b2 , y1 = m2*max.x + b2, lwd = 3, col = "black")}}
return(wacky)
}
```
...
abline(v = tlp, col = alpha("#64A400", 0.8), cex = 3)
polygon(x = c(tlp.lower, tlp.upper, tlp.upper, tlp.lower), y = c(ymax +1.5, ymax +1.5, ymin -1.5, ymin -1.5), col = alpha("#64A400", 0.3), border = alpha("#64A400", 0.7))
abline(a = b, b = m, lwd = 3, col = "black")
...
for(j in 1:length(xvar)){
  if(j < p){
    segments(x0 = -10, x1 = p, y0 = (-10*m) + b , y1 = m2*((b - b2)/(m2 - m))+b2, lwd = 3, col = "black")
  }
  else{
    segments(x0 = p, x1 = max.x, y0 = m2*((b - b2)/(m2 - m))+b2, y1 = b2, lwd = 3, col = "black")
  }
    }
return(wacky)
}
...
```{r}
o.mod<-lme(fh~mpa + year.month + site, random=~1|id, data=seg.adfa.subset)
os.mod<-segmented.default(o.mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '.coef' argument)
summary.segmented(os.mod)
...
No threshold able to be identified (segmented.default returns error)
```{r}
#o.mod<-lme(temp.max~mpa + year.month + site, random=~1|id, data=seg.adfa.subset)
#os.mod<-segmented.default(o.mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '.coef' argument)
#summary.segmented(os.mod)
...
### Flame Duration
```{r}
o.mod<-lme(fd~mpa + year.month + site, random=~1|id, data=seg.adfa.subset)
os.mod<-segmented.default(o.mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '.coef' argument)
summary.segmented(os.mod)
...
### Time to First Glow
```{r}
o.mod<-lme(ttfg~mpa + year.month + site, random=~1|id, data=seg.adfa.subset)
os.mod<-segmented.default(o.mod, ~mpa, npsi=list(mpa=1))
#summarizing results (note the '.coef' argument)
summary.segmented(os.mod)
...
For threshold values from above segmented regressions and the 95% CI (+/- 1.96*SE); in the following order (repeated twice; once for CEME, then ADFA): PC2, Prop. Ignite, GTI, FH, PFG, FD, MT, GD, TTFG 
```{r}
psi <- c(60.30118, 91.87861, 119.9111, 57.1915, 68.05055, 49.68505, 55.57693, 63.74955, 59.89132, 62.65922, 43.14614, 98.27882, 79.50582, 65.84311, 46.55515, 98.27882, 65.82738, 78.53595)
se.psi <- c(6.658953, 5.592003, 22.37689, 4.920527, 9.347637, 7.83918, 6.153706, 9.380537, 7.690371, 4.290069, 2.192261, 10.12274, 5.414246, 6.058802, 6.604431, 8.924882, 7.186473, 6.963152)
lwr.psi <- psi - 1.96*se.psi
upr.psi <- psi + 1.96*se.psi
psi
lwr.psi
upr.psi
```
...
```{r}
jpeg(here('figures', 'main-figures', 'Fig2_segmented.regressionsNEW.jpg'), width = 1200, height = 900, res = 300)
par(mfrow = c(2,2))
par(mar = c(4,5,3,2))
par(font.lab = 2)
...
draw.circle(60.30118, -3.84, 2.4, col = 'black') # PC2 Threshold
segments(x0 = 47.24963, x1 = 73.35273, y0 = -3.84, y1 = -3.84, col = 'black')
text(x = 60.30118, y = -3.44, labels = c("Consumability"), cex = 1)
...
abline(v = mean_lfm_ceme, col = alpha("#64A400", 0.7), cex = 3)
polygon(x = c(lwr_lfm_ceme, upr_lfm_ceme, upr_lfm_ceme, lwr_lfm_ceme), y = c(86 +0.2*86, 86 +0.2*86, 0-22, -22), col = alpha("#64A400", 0.3), border = alpha("#64A400", 0.6))
legend('topleft', legend = c("September 2020", "January 2020", "December 2019"),
       col = c("#c1121f",'#669BBC',"#b08ba5"), pch = 16, cex=1)
for(z in 1:length(seg.ceme.subset[,'lfm'])){
  if(z < summary.segmented(os.mod.b)$psi[2]){
    segments(x0 = 0, x1 = summary.segmented(os.mod.b)$psi[2], y0 = intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[1]], y1 = slope(os.mod.b, .coef = fixef(os.mod.b))[[1]][[1]]*((intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]] - intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[1]])/(slope(os.mod.b, .coef = fixef(os.mod.b))[[1]][[1]] - slope(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]]))+intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[1]], lwd = 3, col = "black")
  }else{
    segments(x0 = summary.segmented(os.mod.b)$psi[2], x1 = max.lfm, y0 = slope(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]]*((intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[1]] - intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]])/(slope(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]] - slope(os.mod.b, .coef = fixef(os.mod.b))[[1]][[1]]))+intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]] , y1 = slope(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]]*max.lfm + intercept(os.mod.b, .coef = fixef(os.mod.b))[[1]][[2]], lwd = 3, col = "black")}}
draw.circle(91.87861, -7, 2.4, col = 'black') # Prop ignite threshold
segments(x0 = 80.91828, x1 = 102.83894, y0 = -7, y1 = -7, col = 'black')
text(x = 93.87861, y = -2, labels = c("Prop. Ig."), cex = 1)
draw.circle(119.91110, -20, 2.4, col = 'black') # Glow to ignition threshold
segments(x0 = 76.05240, x1 = 163.76980, y0 = -20, y1 = -20, col = 'black')
text(x = 119.91110, y = -15, labels = c("GTI"), cex = 1)
draw.circle(57.19150, -12, 2.4, col = 'black') # Flame height threshold
segments(x0 = 47.54727, x1 = 66.83573, y0 = -12, y1 = -12, col = 'black')
text(x = 57.19150, y = -7, labels = c("FH"), cex = 1)
draw.circle(68.05055, -16, 2.4, col = 'black') # Post-flame glow threshold
segments(x0 = 49.72918, x1 = 86.37192, y0 = -16, y1 = -16, col = 'black')
text(x = 75.05055, y = -11, labels = c("PFG"), cex = 1)
draw.circle(49.68505, -20, 2.4) # Flame duration threshold
segments(x0 = 34.32026, x1 = 65.04984, y0 = -20, y1 = -20, col = 'black')
text(x = 44.68505, y = -15.25, labels = c("FD"), cex = 1)
draw.circle(55.57693, -2, 2.4, col = 'black') # Maximum temp. threshold
segments(x0 = 43.51567, x1 = 67.63819, y0 = -2, y1 = -2, col = 'black')
text(x = 46.57693, y = 2.5, labels = c("Max. T."), cex = 1)
draw.circle(63.74955, 5.75, 2.4) # Glow duration threshold
segments(x0 = 45.36370, x1 = 82.13540, y0 = 5.75, y1 = 5.75, col = 'black')
text(x = 52.74955, y = 8.75, labels = c("GD"), cex = 1)
draw.circle(59.89132, 13.5, 2.4) # Time to first glow threshold
segments(x0 = 44.81819, x1 = 74.96445, y0 = 13.5, y1 = 13.5, col = 'black')
text(x = 49.89132, y = 17.5, labels = c("TTFG"), cex = 1)
...
draw.circle(62.65922, -4.6, 1.25, col = 'black') # PC2 Threshold
segments(x0 = 54.25068, x1 = 71.06776, y0 = -4.6, y1 = -4.6, col = 'black')
text(x = 62.65922, y = -4.2, labels = c("Consumability"), cex = 1)
...
abline(v = mean_lfm_adfa, col = alpha("#64A400", 0.7), cex = 3)
polygon(x = c(lwr_lfm_adfa, upr_lfm_adfa, upr_lfm_adfa, lwr_lfm_adfa), y = c(75 +0.2*75, 75 +0.2*75, 0-22, -22), col = alpha("#64A400", 0.3), border = alpha("#64A400", 0.6))
...
draw.circle(43.14614, -17, 1.25, col = 'black') # Prop ignite threshold
segments(x0 = 38.84931, x1 = 47.44297, y0 = -17, y1 = -17, col = 'black')
text(x = 43.14614, y = -12.5, labels = c("Prop. Ig."), cex = 1)
draw.circle(98.27882, -15, 1.25, col = 'black') # Glow to ignition threshold
segments(x0 = 78.43825, x1 = 118.11939, y0 = -15, y1 = -15, col = 'black')
text(x = 98.27882, y = -10.5, labels = c("GTI"), cex = 1)
draw.circle(79.50582, 2, 1.25, col = 'black') # Flame height threshold
segments(x0 = 68.89390, x1 = 90.11774, y0 = 2, y1 = 2, col = 'black')
text(x = 79.50582, y = 6.5, labels = c("FH"), cex = 1)
draw.circle(65.84311, -18, 1.25, col = 'black') # Post-flame glow threshold
segments(x0 = 53.96786, x1 = 77.71836, y0 = -18, y1 = -18, col = 'black')
text(x = 62.84311, y = -13.5, labels = c("PFG"), cex = 1)
draw.circle(46.55515, -7.75, 1.25) # Flame duration threshold
segments(x0 = 33.61047, x1 = 59.49983, y0 = -7.75, y1 = -7.75, col = 'black')
text(x = 45.55515, y = -3.25, labels = c("FD"), cex = 1)
draw.circle(98.27882, -4, 1.25, col = 'black') # Maximum temp. threshold
segments(x0 = 80.78605, x1 = 115.77159, y0 = -4, y1 = -4, col = 'black')
text(x = 98.27882, y = 0.5, labels = c("Max. T."), cex = 1)
draw.circle(65.82738, -2.5, 1.25, col = 'black') # Glow duration threshold
segments(x0 = 51.74189, x1 = 79.91287, y0 = -2.5, y1 = -2.5, col = 'black')
text(x = 64.42738, y = 2, labels = c("GD"), cex = 1)
draw.circle(78.53595, -11, 1.25, col = 'black') # Time to first glow threshold
segments(x0 = 64.88817, x1 = 92.18373, y0 = -11, y1 = -11, col = 'black')
text(x = 78.53595, y = -6.5, labels = c("TTFG"), cex = 1)
...
segments(x0 = p.1, x1 = max.lfm.c, y0 = m2.1*((b.1 - b2.1)/(m2.1 - m.1))+b2.1 , y1 = m2.1*max.lfm.c + b2.1, lwd = 4, col = "black")
```
...
segments(x0 = p.2, x1 = max.lfm.c, y0 = m2.2*((b.2 - b2.2)/(m2.2 - m.2))+b2.2 , y1 = m2.2*max.lfm.c + b2.2, lwd = 4, col = "black")
```
...
segments(x0 = p.3, x1 = max.lfm.c, y0 = m2.3*((b.3 - b2.3)/(m2.3 - m.3))+b2.3 , y1 = m2.3*max.lfm.c + b2.3, lwd = 4, col = "black")
```
...
segments(x0 = p.4, x1 = max.lfm.c, y0 = m2.4*((b.4 - b2.4)/(m2.4 - m.4))+b2.4 , y1 = m2.4*max.lfm.c + b2.4, lwd = 4, col = "black")
```
...
# Supp Figure
```{r}
jpeg(here('figures', 'supp-figures', 'FigS2_seg.reg.by.dates.jpg'), width = 1500, height = 1950, res = 150)
par(mfrow = c(4,3))
par(font.lab = 2)
...
### PC2
sig (p = 0.017856280) 0.1546915 (-6.46)
```{r}
...
Ignitability Metrics:
### Time to Ignition
SIGNIFICANT (p = 0.0015312), 0.3219623 (-3.105954)
```{r}
...
Combustibility Metrics:
### Flame Height
SIGNIFICANT (p = 0.003973) 0.1275757 (-7.84)
```{r}
...
### PC1
SIGNIFICANT (p = 0.001931591) 0.4268678 (-2.34)
```{r}
...
### PC2
SIGNIFICANT (p = 0.002949292) 0.1742132 (-5.74)
```{r}
...
Ignitability Metrics:
### Time to Ignition
SIGNIFICANT (p = 6.4785 x 10^-6) 0.3814873 (-2.62)
```{r}
...
### Glow to Ignition
SIGNIFICANT (p = 0.0070038) 0.4280231 (-2.336)
```{r}
...
Combustibility Metrics:
### Flame Height
SIGNIFICANT (p = 0.002695) 0.3761366 (-2.659)
```{r}
...
sig.p <- c('Yes (p = 0.0019)','Yes (p = 0.0029)','Yes (p < 0.0005)','No','Yes (p = 0.0070)','Yes (p = 0.0027)','No','No','No','No','Yes (p = 0.013)', '6/11 Significant', 'No', 'Yes (p = 0.018)','Yes (p = 0.0015)','No','No','Yes (p = 0.0040)','No','No','No','No','No', '3/11 Significant')
threshold <- c('0.4269','0.1742','0.3815','0.1063','0.4280','0.3761','0.1848','0.1176','8.3102','0.1611','0.2488', 'Avg: 0.3393','0.1001','0.1547','0.3220','0.1196','0.1001','0.1276','0.1236','0.1001','0.1482','0.1508','0.1124', 'Avg: 0.2014')
threshold.mpa <- c('-2.34','-5.74','-2.62','-9.41','-2.34','-2.66','-5.41','-8.50','-0.12','-6.21','-4.02', 'Avg: -2.95','-9.99','-6.46','-3.11','-8.36', '-9.99','-7.84','-8.09', '-9.99','-6.75','-6.63','-8.90', 'Avg: -4.96')
seg.reg.table.df <- data.frame(dep.var, spp, sig.p, threshold, threshold.mpa)
```

Kable:
```{r}
seg.reg.table.df %>% 
   mutate(dep.var = cell_spec(dep.var, 'html', color = 'black', bold = T), spp = cell_spec(spp, 'html', color = 'black'), sig.p = cell_spec(sig.p, 'html', color = 'black'), threshold = cell_spec(threshold, 'html', color = 'black'), threshold.mpa = cell_spec(threshold.mpa, 'html', color = 'black')) %>% 
  kable(format = 'html', escape = F, col.names = c('Dependent Variable', 'Species', 'Significant?','Threshold (-1/MPa)', 'Threshold (MPa)'), caption = '-1/MPa') %>% 
  kable_styling(bootstrap_options = c('hover', 'bordered', 'condensed'), fixed_thead = T) %>% 
  row_spec(c(1:3, 5:6, 11, 14:15, 18), bold = T) %>% 
  row_spec(c(4, 7:10, 13, 16:17, 19:23), bold = F) %>% 
  row_spec(c(12, 24), background = "#D3D3D3", bold = T) %>%
  save_kable(here('figures', 'supp-figures', 'TabS6_inverse.mpa.segreg.summary.html'))
```
