...
os.mod.a<-segmented.lme(o.mod.a, ~lfm, npsi=list(lfm=1), )
#summarizing results (note the '.coef' argument)
#slope(os.mod.a, .coef = fixef(os.mod.a))[[1]][[1]]
...
os<-segmented.lme(o, ~lTime, random=list(Subject=pdDiag(~1+lTime+U)), 
...
segmented.mod <-segmented.default(out.lm, ~lfm, npsi=list(lfm=1))#1 breakpoint for x
segmented.mod <-segmented(out.lm, seg.Z = ~lfm, npsi=list(lfm=1))
class(segmented.mod)
slope(segmented.mod)
```
...
Ignitability Metrics:
### Time to Ignition
SIGNIFICANT (p = 0.01297), 0.222 (-4.504)
```{r}
...
### Prop. Ignite (5%)
SIGNIFICANT (p = 0.0397), 0.1207 (-8.285)
```{r}
...
Combustibility Metrics:
### Flame Height
SIGNIFICANT (p = 0.0153) 0.1237 (-8.08)
```{r}
...
### Time to First Glow
SIGNIFICANT (p = 0.03727) 0.179 (-5.5897)
```{r}
...
### PC1
SIGNIFICANT (p = 0.00545) 0.4303 (-2.324)
```{r}
...
### PC2
SIGNIFICANT (p = 0.007566) 0.1738 (-5.75)
```{r}
...
Ignitability Metrics:
### Time to Ignition
SIGNIFICANT (p = 8.1962 x 10^-5) 0.4175 (-2.395)
```{r}
...
### Glow to Ignition
SIGNIFICANT (p = 0.00811) 0.4322 (-2.314)
```{r}
...
Combustibility Metrics:
### Flame Height
SIGNIFICANT (p = 0.00492) 0.3885 (-2.574)
```{r}
...
sig.p <- c('Yes (p = 0.0055)','Yes (p = 0.0076)','Yes (p < 0.0005)','No','Yes (p = 0.0081)','Yes (p = 0.0049)','No','No','No','No','No', '5/11 Significant', 'No', 'No','Yes (p = 0.013)','Yes (p = 0.040)','No','Yes (p = 0.015)','No','No','No','No','Yes (p = 0.037)', '4/11 Significant')
threshold <- c('0.4303','0.1738','0.4175','0.1131','0.4322','0.3885','0.1046','0.1734','8.5786','0.1580','0.2488', 'Avg: 0.3685','0.1001','0.1001','0.222','0.1207','0.1001','0.1237','0.1','0.1001','0.1','0.1001','0.179', 'Avg: 0.1614')
threshold.mpa <- c('-2.32','-5.75','-2.40','-8.84','-2.31','-2.57','-9.56','-5.77','-0.12','-6.33','-4.02', 'Avg: -2.71 ','-9.99','-9.99','-4.50','-8.29','-9.99','-8.08','-10','-9.99','-10','-9.99','-5.59', 'Avg: -6.20')
...
seg.reg.table.df %>% 
  ...
  row_spec(c(1:3, 5:6, 15:16, 18, 23), bold = T) %>% 
  row_spec(c(4, 7:11, 13:14, 17, 19:22), bold = F) %>% 
