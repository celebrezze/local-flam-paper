...
```{r}
o<-lme(tti~mpa, random=~1|id, data=seg.data.subset)
os<-segmented.default(o, ~mpa, npsi=list(mpa=1))

#summarizing results (note the '.coef' argument)
slope(os, .coef=fixef(os))
plot.segmented(os, "mpa", .coef=fixef(os), conf.level=.95)
confint.segmented(os, "mpa", .coef=fixef(os))
```
