...
epi.flamdata_noignitions <- read_csv(here("processed-data", "compiled-datasets", "LOCAL",  "local_flam_data_all.csv")) %>%
  mutate(LFM = lfm.NAs.imputed) %>%
  #filter(year == 2020) %>%
  filter(model == "EPI") %>%
  filter(ignition == 0)
...
write_csv(mem.data, here("processed-data", "compiled-datasets", "LOCAL", "mem.data.local.epi.alldates.csv"))
```
...
```{r}
mpa_mod_fig.mpa <- lmer(PC1 ~ mpa + spp*year.month + (1 | individual), data = scaled.mem.data.alldates.noNAs) #mpa model

m3 <- lmer(PC1 ~ spp*year.month + ww.flam.sample + (1 | individual), data = scaled.mem.data.alldates.noNAs) #ww model
  
m6 <- lmer(PC1 ~ dw.flam.sample + spp*year.month + (1 | individual), data = scaled.mem.data.alldates.noNAs) #dw model

m7 <- lmer(PC1 ~ mpa + lfm.NAs.imputed + spp*year.month + (1 | individual), data = scaled.mem.data.alldates.noNAs) #mpa nad lfm model
m7

m10 <- lmer(PC1 ~ lfm.NAs.imputed + spp*year.month + (1 | individual), data = scaled.mem.data.alldates.noNAs) #lfm model

m11 <- lmer(PC1 ~ excess_water + spp*year.month + (1 | individual), data = scaled.mem.data.alldates.noNAs)

m12 <- lmer(PC1 ~ excess_water + mpa + lfm.NAs.imputed + spp*year.month + (1 | individual), data = scaled.mem.data.alldates.noNAs)
m12
```
...
tab_model(mpa_mod_fig.mpa , m7, m10, m3, m6, m11, m12,
          show.reflvl = TRUE, 
          digits = 3, 
          show.aic = TRUE, 
          show.ci = FALSE,
...
ggsave(here("scripts", "LOCAL", "figures", "season_plot.jpg"),
       plot = season_plot, 
...
ggsave(here("scripts", "LOCAL", "figures", "precip_plot.jpg"),
       plot = precip_plot, 
       width = 6, height = 4, dpi = 300)
```
#PC1 by LFM
...
ggsave(here("scripts", "LOCAL", "figures", "timing_precip_plot.jpg"),
       plot = timing_plot, 
       width = 6, height = 4, dpi = 300)
```
...
ggsave(here("scripts", "LOCAL", "figures", "scatterplot_ranef_all.jpg"),
       plot = plot_legend, 
...
ggsave(here("scripts", "LOCAL", "figures", "scatterplot_noremef.jpg"),
       plot = plot_legend, 
       width = 6.5, height = 4.5, dpi = 300)
```
...
```{r}
ggarrange(tti.mpa2, gd.mpa2, fh.mpa2, pc1.mpa2, season_plot.adfa, season_plot.ceme, common.legend = TRUE)

ggsave("spp.and.season.effects.png", height = 7, width = 11)

spp.effects.plots <- ggarrange(tti.mpa2, gd.mpa2, fh.mpa2, pc1.mpa2, common.legend = TRUE, legend = c("right"))
spp.effects.plots

szn.effects.plots <- ggarrange(season_plot.ceme, season_plot.adfa, common.legend = TRUE, legend = c("bottom"))
szn.effects.plots

plot_grid(spp.effects.plots, szn.effects.plots, ncol = 1, rel_heights = c(3, 2), labels = c("A", "B"))
ggsave("spp.and.season.effects2.png", height = 10, width = 7)
```
...
ggsave(here("scripts", "LOCAL", "figures", "dw_plot.jpg"),
       plot = precip_plot, 
       width = 6, height = 4, dpi = 300)
```
...
```{r}
tmp %>% 
  ggplot(aes(y = prop.ignite.bins10_nodate, x = lfm.unscaled, fill = tlp_split)) +
  facet_wrap(~spp) +
  geom_boxplot()
```
```{r}
tmp %>% 
  ggplot(aes(y = prop.ignite.bins10_nodate, x = mpa.unscaled, fill = tlp_split)) +
  facet_wrap(~spp) +
  geom_boxplot() +
  ylab("Ignition Likelihood")
```
#####bins 5
```{r}
tmp %>% 
  ggplot(aes(y = prop.ignite.bins5_nodate, x = lfm.unscaled, fill = tlp_split)) +
  facet_wrap(~spp) +
  geom_boxplot()
```
...
```{r}
tmp %>% 
  ggplot(aes(y = prop.ignite.bins5_nodate, x = mpa.unscaled, fill = tlp_split)) +
  facet_wrap(~spp) +
  geom_boxplot()
```
#####PC1
...
# Separating by Species
mpa_plot_spp <- ggplot() + 
  geom_point(data= scaled.mem.data.alldates.noNAs, aes(mpa, PC1, color = spp), size = .5) + 
