...
#Parsing out the variables of interest for the mixed effects modeling
clean.mem.data.dependant.alldates <- clean.mem.data.alldates %>% 
  dplyr::select(c('PC1', 'PC2', 'PC3', 'PC4', 'tti', 'fh', 'gd', 'fd', 'temp.max', 'flam.index', 'year', 'month', 'prop.ignite', 'prop.ignite.bins5', "ignition"))
 
scaled.mem.data.predictors.alldates.adfa <- clean.mem.data.alldates %>%
  filter(spp == "ADFA") %>% 
  dplyr::select(lfm.NAs.imputed, lfm.outliers.out, mpa, dw.flam.sample, 
...
write.csv(seg.data.subset.alldates, here('processed-data', 'mem.data.subset.alldates'))
```

```{r}
scaled.mem.data.predictors.alldates.ceme %>% 
  ggplot(aes(x = mpa, y = mpa.unscaled)) +
  geom_point()
```
