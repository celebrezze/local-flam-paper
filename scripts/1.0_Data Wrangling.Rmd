...
range(ignite.only.adfa$tti)
range(ignite.only.adfa$fh)
#chack for NAs
ignite.only.adfa <- ignite.only.adfa %>%
...
range(ignite.only.adfa$flam.index)
```
...
clean.mem.data <- mem.data.raw[,c('PC1', 'PC2', 'PC3', 'PC4', 'spp', 'lfm.NAs.imputed', 'lfm.outliers.out', 'mpa', 'dw.flam.sample', 'year.month', 'individual', 'site','sample.wt', 'ww.flam.sample', 'start.temp',  'year', 'month', 'tti', 'fh', 'gd', 'fd', 'gti', 'pfg', 'ttfg', 'temp.max','flam.index', 'precip.2mo', 'prop.ignite.bins5_nodate', 'prop.ignite.bins10_nodate')]
```
...
seg.data.subset <- clean.mem.data %>% 
  mutate(lfm = lfm.NAs.imputed, 
         id = individual) %>% 
 dplyr::select(id, PC1, PC2, PC3, mpa, lfm, ww.flam.sample, dw.flam.sample, site, sample.wt, spp, tti, fh, fd, gd, gti, pfg, temp.max, year.month, prop.ignite.bins10_nodate, prop.ignite.bins5_nodate, ttfg)
...
clean.mem.data.alldates <- clean.mem.data.alldates %>% 
  select(c('PC1', 'PC2', 'PC3', 'PC4', 'tti', 'fh', 'gd', 'fd', 'temp.max', 'flam.index', 'year', 'month', 'prop.ignite', 'prop.ignite.bins5', "ignition", 'spp', 'year.month', 'year', 'month', 'individual', 'site', 'start.temp', 'precip.2mo','mpa.unscaled', 'lfm.unscaled', 'dw.flam.sample.unscaled', 'lfm.NAs.imputed.unscaled', 'excess_water.unscaled', 'hydration')) %>% 
  arrange(individual)
...
write.csv(scaled.mem.data.alldates.noNAs, here('processed-data', 'mem.data.subset.alldates'))
```