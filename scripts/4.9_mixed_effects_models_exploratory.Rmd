...
mem.table2.df %>%
  ...
  kable(format = 'html', escape = F, col.names = c('Dependent Variable', 'Model Fixed Effects', 'Water Estimate','Change in AIC', expression('Marginal R2/Conditional R2'))) %>% 
  kable_styling(bootstrap_options = c('hover', 'bordered', 'condensed'), fixed_thead = T) %>% 
  row_spec(c(1, 7, 13, 19), background = "#D3D3D3", bold = T, font_size = 16) %>% 
  row_spec(c(2, 8, 14, 15, 16, 20), bold = T) %>% 
  save_kable(here('figures', 'Table2_MEM_kable.html'))
```
