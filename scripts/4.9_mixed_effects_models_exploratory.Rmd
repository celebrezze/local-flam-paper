...
```
```{r}
a <- lmer(PC1 ~ dw.flam.sample + precip.2mo + site + spp + (1 | individual), data = scaled.mem.data.alldates.noNAs)
summary(a)
performance::multicollinearity(a)

ggplot(scaled.mem.data.alldates.noNAs, aes(y = dw.flam.sample, x = as.factor(precip.2mo))) +
  geom_boxplot() +
  facet_wrap(~spp)
```
