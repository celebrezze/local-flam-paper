...
ignite.only.ceme <- ignite.only.ceme %>%
  ...
  impute_median_at(c("ttfg", "gd", "gti", "fd", "pfg","temp.max", "start.temp", "ignition.temp", "temp.change", "prop.ignite", "fh", "tti")) 
```
...
### USE THIS FOR PCA: Mirroring Max's PCA dataset (Mac_PCA_DATE.Rmd)

```{r, include=FALSE}
ignite.only.ceme.PCA <- ignite.only.ceme %>%
  mutate(mpa = mpa * -1) 
```
...
```{r, include=FALSE}
ignite.only.adfa.PCA <- ignite.only.adfa %>%
  mutate(mpa = mpa * -1)
colSums(is.na(ignite.only.adfa.PCA))
```
...
pca.data.local.noignites <- rbind(no.ignites.adfa, no.ignites.ceme) %>%
  mutate(mpa = mpa * -1) 
```


```{r}
pca.data.local.ignites <- rbind(ignite.only.ceme.PCA, ignite.only.adfa.PCA)

pca.data.local <- bind_rows(pca.data.local.ignites, no.ignites.all)

colSums(is.na(pca.data.local))

write_csv(pca.data.local, here("processed-data",  "local_flam_data_all.csv"))
```
