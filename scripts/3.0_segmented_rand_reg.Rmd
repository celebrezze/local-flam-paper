...
```{r}
rsegplot.lfm <- function(model, dataframe, x.variable = 'lfm', y.variable, max.x = max.lfm, x.lab = 'Live Fuel Moisture (%)', y.lab = NULL, tlp, tlp.upper, tlp.lower) {
...
```{r}
rsegplot.mpa <- function(model, dataframe, x.variable = 'mpa', y.variable, max.x = max.mpa, x.lab = 'Water Potential (MPa)', y.lab = NULL, tlp, tlp.upper, tlp.lower) {
...
## LFM x PC1
Tried out adding TLP and standard error for TLP below, and seemed to work out okay
```{r}
...
rsegplot.lfm(os.pc1.lfm.c, seg.ceme.subset, y.variable = 'PC1',y.lab = "PC1", tlp = mean_lfm_ceme, tlp.lower = lwr_lfm_ceme, tlp.upper = upr_lfm_ceme)
```
...
rsegplot.lfm(os.pc1.lfm.a, seg.adfa.subset, y.variable = 'PC1',y.lab = "PC1", tlp = mean_lfm_adfa, tlp.lower = lwr_lfm_adfa, tlp.upper = upr_lfm_adfa)
```
