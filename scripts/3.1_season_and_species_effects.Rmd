...
pc1.vs.mpa.adfa <- precip_groups_ranis.adfa  %>%
  ...
  annotate('text', x = -7.3, y = -3.75, label = 'More \n Combustible \n & Ignitable', fontface = 2, size = 6) + # THIS IS FOR TEXT TO RIGHT OF ARROW
...
pc1.vs.mpa.adfa.avgsite <- precip_groups_ranis  %>%
  ...
  annotate('text', x = -8, y = -3.75, label = 'More \n Combustible \n & Ignitable', fontface = 2, size = 6) +
  geom_point(alpha = .65, aes(color = season), size = 2) +
...
pc2.vs.mpa.adfa <- precip_groups_ranis.adfa  %>%
  ...
  annotate('text', x = -7.3, y = -4.2, label = 'Less \n Consumable', fontface = 2, size = 6) +
  labs(x = "Water Potential (-MPa)", 
       y = "Consumability Proxy", 
       fill = "Season",
       color = "Season") +
  scale_y_continuous(limits=c(-5,3.25), breaks=c(-5,-2.5,0,2.5,5)) +
  th +
  guides(color = guide_legend(override.aes = list(linetype = 0, shape = 16))) +
  theme(legend.position = "none", 
        legend.title = element_text(face = "bold"),
        ...
        axis.text.y = element_text(size = 14)) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  annotate('text', x = -9.7, y = 3, label = 'c', size = 12, face = 'bold') +
  scale_color_manual(values=c("#c1121f","#669BBC"))
pc2.vs.mpa.adfa
```
...
pc2.vs.mpa.ceme <- precip_groups_ranis.ceme  %>%
  ...
  theme(legend.position = "none", 
        ...
        axis.text = element_blank()) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  annotate('text', x = -9.7, y = 3, label = 'd', size = 12, face = 'bold') +
  scale_color_manual(values=c("#c1121f","#669BBC"))
pc2.vs.mpa.ceme
```
...
mpa.vs.lfm.adfa <- mpa_rel_df %>% 
  ...
  geom_abline(slope = 0.170, intercept = 0.297,
              ...
              show.legend = FALSE) +
  labs(x = "Water Potential (MPa)", 
       y = "LFM (Scaled, Effects Removed)", 
       fill = "Season",
       color = "Season") +
  scale_y_continuous(limits=c(-2.5,2), breaks=c(-2.5, -2, -1.5, -1,-.5, 0, 1, .5, 1.5, 2, 2.5 )) +
  th +
  guides(color = guide_legend(override.aes = list(linetype = 0, shape = 16))) +
  theme(legend.position = "none", 
        legend.title = element_text(face = "bold"),
  ...
  guides(color = guide_legend(override.aes = list(size = 3))) +
  annotate('text', x = -9.7, y = 1.75, label = 'e', size = 12, face = 'bold') +
  scale_color_manual(values=c("#c1121f","#669BBC"))
mpa.vs.lfm.adfa
```
...
mpa.vs.lfm.ceme <- mpa_rel_df %>% 
  ...
  guides(color = guide_legend(override.aes = list(size = 3))) +
  annotate('text', x = -9.7, y = 1.75, label = 'f', size = 12, face = 'bold') +
  scale_color_manual(values=c("#c1121f","#669BBC"))
mpa.vs.lfm.ceme
```
...
legend.sz <- get_legend(
  ...
  ggplot() + 
  geom_point(data= seg.data.subset.alldates, aes(mpa.unscaled, PC1, color = season), 
             size = 5, 
             alpha = .8) +
    labs(color = "Season") +
    scale_color_manual(values = c('#c1121f', '#669bbc')) +
...
#         pc1.vs.mpa.adfa + theme(plot.margin = unit(c(.25, 0, 0, 0), "cm")), 
#        # NULL,
#         pc1.vs.mpa.ceme + theme(plot.margin = unit(c(.25, 0, 0, .25), "cm")),
#         pc2.vs.mpa.adfa + theme(plot.margin = unit(c(.25, 0, 0, 0), "cm")), 
#       #  NULL,
#         pc2.vs.mpa.ceme + theme(plot.margin = unit(c(.25, 0, 0, .25), "cm")),
#         mpa.vs.lfm.adfa + theme(plot.margin = unit(c(.25, 0, 0, 0), "cm")),
#       #  NULL,
#         mpa.vs.lfm.ceme + theme(plot.margin = unit(c(.25, 0, 0, .25), "cm")),
#         align='vh', 
#         axis = 'lr',
#         vjust=0,
#         rel_widths = c(1, 1),
...
plot_legend.sz.mem <- cowplot::plot_grid(legend.sz, scatterplot.sz.mem, ncol = 1, rel_heights = c(.7,10))
plot_legend.sz.mem
...
plot.sz.lfm <- pc1.vs.lfm.adfa +
        pc1.vs.lfm.ceme + 
        pc2.vs.lfm.ceme + 
        pc2.vs.lfm.adfa + 
  plot_layout(ncol = 2, heights = c(1, 1, 1), widths = c(1, 1))

plot.sz.mem <- as_grob(plot.sz.lfm)
